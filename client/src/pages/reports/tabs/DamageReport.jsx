...
const DamageReport = ({ data, loading, activeSubTab }) => {
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <div className="w-10 h-10 border-4 border-red-600/20 border-t-red-600 rounded-full animate-spin"></div>
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Generating Damage Report...</p>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-4">
        ...
        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No Damage Data Found</p>
      </div>
    );
  }
        ...
        <div key={lIdx} className="bg-white border border-slate-300 rounded shadow-sm overflow-hidden text-slate-800">
          <div className="bg-slate-100 border-b border-slate-300 px-4 py-3 flex items-center justify-between">
            <h3 className="font-bold uppercase text-sm">
              {activeSubTab === 'category_wise' ? 'Category Summary' : `Damage at: ${location?.canteenname}`}
            </h3>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead className="bg-slate-50 border-b border-slate-300 text-slate-700">
                <tr>
                  ...
                  <th className="px-4 py-2 text-xs font-bold uppercase border-r border-slate-200 text-center w-32">Qty Damaged</th>
                  <th className="px-4 py-2 text-xs font-bold uppercase border-r border-slate-200 text-right w-36">Loss Value</th>
                  <th className="px-4 py-2 text-xs font-bold uppercase text-center w-32">CAUSE</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <React.Fragment key={gIdx}>
                    ...
                    {group?.productlist?.map((item, pIdx) => (
                      <tr key={pIdx} className="border-b border-slate-200 hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2 text-sm text-center border-r border-slate-200 font-bold bg-amber-50/10 text-amber-600">{item?.qty || 0}</td>
                        <td className="px-4 py-2 text-sm font-semibold text-right border-r border-slate-200">₹{(item?.total || 0).toLocaleString()}</td>
                        <td className="px-4 py-2 text-center italic text-[10px] text-slate-500">
                           {item?.remarks || 'Operational Damage'}
                        </td>
                      </tr>
                    ))}
                  ...
                  <td colSpan={5} className="px-0 py-0">
                    <div className="flex justify-end items-center bg-slate-900 p-5 gap-8">
                       <div className="flex items-center gap-2">
                           ...
                           <div>
                               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Total Loss Valuation</p>
